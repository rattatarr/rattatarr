...
-- Recreate background_jobs with updated CHECK constraint to include anime arr job types.
...
CREATE TABLE background_jobs_new (
    ...
    type varchar(255) not null check (type in ('JELLYFIN_SYNC', 'JELLYFIN_ACTIVITY_POLL', 'CSV_IMPORT', 'RADARR_IMPORT', 'RADARR_RATINGS_REFRESH', 'RADARR_ANIME_IMPORT', 'RADARR_ANIME_RATINGS_REFRESH', 'SONARR_IMPORT', 'SONARR_ANIME_IMPORT')),
    primary key (id)
);
