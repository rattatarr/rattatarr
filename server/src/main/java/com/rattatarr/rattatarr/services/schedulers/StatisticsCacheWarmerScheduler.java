...
public class StatisticsCacheWarmerScheduler {
    ...
    public void warmStatisticsCache() {
        try (var ignored = MdcContext.of(Map.of("jobType", "STATISTICS_CACHE_WARM", "jobSource", "SCHEDULER"))) {
            ...
            for (Profile profile : profiles) {
                try {
                    ...
                    logger.warn("Statistics cache warm failed for profile '{}' ({}): {}",
                            profile.name(), profile.id(), e.getMessage(), e);
                    failed++;
                }
            }
