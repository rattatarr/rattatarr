...
public record ProfileStatisticsRequestDTO(
        ...
        @Min(value = 1, message = "Genre over time limit must be at least 1")
        Integer genreOverTimeLimit,

        String profileImageSize
) {
    public ProfileStatisticsRequestDTO {
        if (ratingThreshold == null) {
            ratingThreshold = 7.0f;
        }
        if (minCount == null) {
            minCount = 1;
        }
        ...
        if (directorsLimit == null) {
            directorsLimit = 10;
        }
        if (producersLimit == null) {
            producersLimit = 10;
        }
        if (genreOverTimeLimit == null) {
            genreOverTimeLimit = 5;
        }
